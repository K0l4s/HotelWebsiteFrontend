  ...
  return (
    <div className='navbar'>
      ...
      <div className="buttonGroup">
        ...
        {username ?
          <>
            <button className={path === '/profile' ? 'active' : ''} onClick={() => navigate('profile')}> <TbLogin2 /> {'Xin chào,' + username} </button>
            <button className={path === '/order' ? 'active' : ''} onClick={() => navigate('order')}><IoCreateOutline /> Order của bạn</button>
            <button className={path === '/logout' ? 'active' : ''} onClick={() => navigate('order')}><IoCreateOutline /> Đăng xuất</button>
          </>
          :
          <>
